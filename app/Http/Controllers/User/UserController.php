    ...
    foreach ($timeSheets as $timeSheet) {
        if ($timeSheet->cost_center == 'hrs_worked') {
            ...
            if (count($workTimeParts) >= 2) {
                ...
                $totalAnnualLeave += $decimalHours * 0.073421;
            }
        }
    }
    $leave->total_annual_leave = $totalAnnualLeave;

    // Calculate taken annual leave
    $takenAnnualLeave = 0;
    foreach ($timeSheets as $timeSheet) {
    ...
    foreach ($timeSheets as $timeSheet) {
        if (!empty($timeSheet->cost_center)) {
            ...
            if (in_array('sick_leave', $costCenters)) {
                $sickLeaveCount += $convertToDecimalHours($timeSheet->work_time);
            }
        }
    }
    ...
    foreach ($timeSheets as $timeSheet) {
        if (!empty($timeSheet->cost_center)) {
            ...
            if (in_array('public_holiday', $costCenters)) {
                $publicHolidayCount += $convertToDecimalHours($timeSheet->work_time);
            }
        }
    }
    ...
    foreach ($timeSheets as $timeSheet) {
        if (!empty($timeSheet->cost_center)) {
            ...
            if (in_array('unpaid_leave', $costCenters)) {
                $unpaidLeaveCount += $convertToDecimalHours($timeSheet->work_time);
            }
        }
    }
...
public function showPayslips(Request $request)
{
    ...
    foreach ($payslips as $payslip) {
        ...
        // Append the calculated data to the date ranges
        $dateRanges[] = [
            'start' => $start_date,
            'end' => $end_date,
            'status' => $status,
...
/**
 * Helper method to calculate hours worked, accounting for regular and leave types.
 */
private function calculateHoursWorked($timeSheetsInRange, $leave)
{
    ...
    public function generatePayslipsPdf(Request $request)
    {
        ...
        $pdf = Pdf::loadView('user.payslips_pdf', compact(
            'user_address', 'hourly_rate', 'hrs_worked', 'abn', 'admin',
            'user_name', 'start_date', 'end_date', 'gross_earning',
            'annual_leave', 'currency', 'timeSheets'
        ));
