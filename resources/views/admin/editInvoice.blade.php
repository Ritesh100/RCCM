...
<div class="container-fluid">
    ...
    <form action="{{ route('admin.invoice.update', $invoice->id) }}" id="invoiceForm" method="POST"
            ...
            <div class="row g-3 align-items-center">
                ...
                <div class="col-md-6">
                    ...
                    <select name="invoice_for" id="invoiceFor" class="form-select" required>
                        ...
                        <option value="{{ $user->name }}" data-id="{{ $user->id }}" data-email="{{ $user->email }}" {{
                            $invoice->invoice_for == $user->name ? 'selected' : '' }}>
                            {{ $user->name }}
                        </option>
                        @endforeach
                    </select>
                ...
                <div id="charges">
                    ...
                    <div class="row">
                        <div class="col-md-6">
                            ...
                            <input type="text" class="form-control" id="charge_{{ $index }}_name"
                                name="charges[{{ $index }}][name]" value="{{ $charge_name }}" required>
                        </div>
                        <div class="col-md-6">
                            <label for="charge_{{ $index }}_total" class="form-label">Charge Total</label>
                            <input type="number" class="form-control" id="charge_{{ $index }}_total"
                                name="charges[{{ $index }}][total]" value="{{ $invoice->charge_totals[$index] }}"
                                step="0.01" required>
                        </div>
                    </div>
                    @endforeach
                ...
                <div class="col-md-6">
                    ...
                    <input type="number" class="form-control" id="totalChargeRCs" name="total_charge_rcs" step="0.01"
                        required value="{{$invoice->total_charge}}">
                ...
                <div class="col-md-6">
                    ...
                    <input type="number" class="form-control" id="totalTransferredRCs" name="total_transferred_rcs"
                        step="0.01" value="{{$invoice->total_transferred}}" required>
                </div>
                ...
                <div class="col-md-6">
                    ...
                    <input type="number" class="form-control" id="previousCredits" name="previous_credits" step="0.01"
                        value="{{$invoice->previous_credits}}" required>
                </div>
                ...
                <div class="mb-4" id="imagePreviewContainer" style="display: flex; flex-wrap: wrap; gap: 10px;">
                    ...
                    <div class="image-preview"
                        style="position: relative; width: 100px; height: 100px; margin-bottom: 10px;">
                        <img src="{{ asset('storage/' . $path) }}" alt="Invoice Image"
                            style="width: 100%; height: 100%; object-fit: cover;">
                        <button type="button" class="btn btn-danger btn-sm remove-image" data-index="{{ $index }}"
                            style="position: absolute; top: 0; right: 0; background-color: red; color: white; border: none; padding: 5px;">
